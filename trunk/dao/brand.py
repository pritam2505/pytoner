...
class Brand(core.Dao):
  ...
  def add(self, brand_name):
    if self.exists_by_name(brand_name) is True:
      self._logs.info('brand %s exists !' % (brand_name))
      return False
      
    query = "INSERT INTO brand ( brand_name ) VALUES ( '%s');" % brand_name
  ...
  def _make_list(self, cursor):
    ...
    for row in cursor.fetchall():
      brand = models.Brand(row[0], row[1])
      brands.append(brand)
    return brands
  ...
  def get_by_id(self, brand_id):
    ...
    if cur.rowcount == 1:
      row = cur.fetchone()
      result = models.Brand(brand_id, row[0])
